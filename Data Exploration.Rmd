...
> Whole numbers or common fractions because of standardization

* Why are there more diamonds slightly to the right of each peak than there 
  are slightly to the left of each peak?
  
> Clusters gather towards the right due to more likely sales
  
* Why are there no diamonds bigger than 3 carats?

> Cleaned the dataset; carat increases price significantly which discourages buyers
...
> There is a gap where no prices exist around $1500
...
ggplot(data = diamonds) +
  ...
  coord_cartesian(xlim = c(1750, 2000))
```

> Binwidth will set itself when unset by the code; when zoomed far enough in, the bin appears flat at a single y value
...
```{r}
flights2 <- flights %>%
  mutate(dep_time = ifelse(dep_delay(NA = TRUE) | arr_delay(NA = TRUE), NA, dep_time))
...
> Carat appears to be most important in dictating the price of a diamond; excluding rarer higher carats, greater carats tend to indicate higher prices. The median fair-cut diamond has a higher carat than the median ideal-cut diamond. Since higher carats fetch higher prices, low-quality-cut diamonds could be more expensive overall.
...
```{r, dev = "png"}
ggplot(diamonds, aes(x = carat, y = price, alpha = 0.01)) +
  geom_point()
```
